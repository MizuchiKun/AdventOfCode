...
static i32 HandleInstructions(char *instructions, bool *isMulEnabled, const bool handleDoDont)
{
    ...
    while (hasMoreMulInstructions)
    {
        ...
        i32 mulReturn = regexec(&MulExpression, readHead, MUL_MATCH_LENGTH, mulMatch, 0);
        i32 doReturn = regexec(&DoExpression, readHead, DO_MATCH_LENGTH, doMatch, 0);
        i32 dontReturn = regexec(&DontExpression, readHead, DONT_MATCH_LENGTH, dontMatch, 0);
        ...
        i32 mulMatchStart = mulMatchFound ? mulMatch->rm_so : LINE_LENGTH;
        i32 doMatchStart = doMatchFound ? doMatch->rm_so : LINE_LENGTH;
        i32 dontMatchStart = dontMatchFound ? dontMatch->rm_so : LINE_LENGTH;

        i32 firstMatchStart = min(mulMatchStart, min(doMatchStart, dontMatchStart));
        if (mulMatchFound && mulMatch->rm_so == firstMatchStart)
        {
            if (*isMulEnabled)
            {
                ...
                i32 arg1 = strtol(arg1Start, &endPtr, RADIX);
                i32 arg2 = strtol(arg2Start, &endPtr, RADIX);
                totalMul += arg1 * arg2;
            }
...
static i32 ProcessInstructionsFile(const char *filename, bool handleDoDont)
{
...
static void Part1()
{
    ...
    i32 totalMul = ProcessInstructionsFile(filename, false);
...
static void Part2()
{
    ...
    i32 totalMul = ProcessInstructionsFile(filename, true);
